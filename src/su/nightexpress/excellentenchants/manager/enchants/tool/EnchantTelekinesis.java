...
public class EnchantTelekinesis extends IEnchantChanceTemplate implements PassiveEnchant {
	...
	protected void updateConfig() {
		...
		if (cfg.contains("settings.radius.horizontal")) {
			String i1 = cfg.getString("settings.radius.horizontal", "")
					.replace("%level%", PLACEHOLDER_LEVEL);
			String i2 = cfg.getString("settings.radius.vertial", "")
					.replace("%level%", PLACEHOLDER_LEVEL);
			String i3 = cfg.getString("settings.power", "")
					.replace("%level%", PLACEHOLDER_LEVEL);
	...
	public @NotNull UnaryOperator<String> replacePlaceholders(int level) {
		return str -> super.replacePlaceholders(level).apply(str
				.replace("%radius%", NumberUT.format(this.radHorizon.getValue(level)))
		);
	}
	...
	public boolean use(@NotNull LivingEntity user, int level) {
		...
		.map(entity -> (Item) entity).forEach(item -> {
