...
public class Config extends IConfigTemplate {
    ...
    public void load() {
    	...
    	ENCHANTMENTS_DISABLED = cfg.getStringSet(path + "Disabled").stream()
	...
	public static EnchantTier getTierByChance(@NotNull ObtainType obtainType) {
		Map<EnchantTier, Double> map = getTiers().stream()
				.collect(Collectors.toMap(k -> k, v -> v.getChance(obtainType)));
		return Rnd.get(map);
	}
